...
def comparePerformance(epss, names, name, lw=5, palOrder=None, hueOrder=None, styleOrder=None, legendOrder=None):
    if hueOrder is None:
    ...
    if not palOrder is None:
        palOrder= [pal[p] for p in palOrder]
    ax = sns.lineplot(data=dfm, x='eps', y='Proportion', palette=palOrder, hue='Algorithm', lw=5, style='Algorithm', style_order = styleOrder, hue_order = hueOrder)
...
def compareImages(imgLists, epssLists, labels, indices, targets, name, inverted = True):
...
def table(data, epss, outputSize = 10):
    ...
    for i in range(outputSize):
        for j in range(outputSize):
            bestClassRatio[i,j] = np.round(results[i,j]/np.sum(results[i,:]),2)
...
def conditionNumberComponentwise(net, data, epss, imgs, name=None, type=1):
    with torch.no_grad():
        ...
        for i, (img, _) in enumerate(data):
            ...
            conds.append(np.linalg.norm(B, ord=np.inf) / np.linalg.norm(output, ord=np.inf)) 
